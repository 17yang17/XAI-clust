...
# 绘制全部数值变量的主要贡献变量图表
dr.plot_num_main_contributors(n_contributors=5,  # 显示前5个主要贡献变量
...
# 绘制主要贡献变量图表
dr.plot_num_main_contributors(dim_idx=3, output_path=os.path.join("img_MAC", "其中一个数值主成分的主要贡献变量图表dim_red_main_contributors.jpg"))
...
# 解释从分类变量中提取的成分:结果保存
dr.cat_main_contributors_stats().to_csv('data_MAC/data_澳门_modified_解释从分类变量中提取的成分.csv', index=False)

# 在分类变量的降维过程中（如MCA）后，有几个原始变量与新的构造变量（主成分）高度相关，默认相关性阈值为0.14
# 绘制分类变量成分分布
dr.plot_cat_main_contributor_distribution(dim_idx=0, 
                                          thres=0.2,  # 提高相关性阈值
                                          n_contributors=6,  # 增加显示数量
                                          output_path=os.path.join("img_MAC", "分类变量成分分布dim_red_cat_component.jpg"))
print("已完成分类变量成分分布的生成。")

df_t.to_csv('data_MAC/data_澳门_modified_preprocessed_dim_red_output.csv', index=False)
...
# 比较聚类内均值与全局均值:结果保存
cl.compare_cluster_means_to_global_means().to_csv('data_MAC/data_澳门_modified_比较聚类内均值与全局均值.csv', index=False)
...
cl.plot_cluster_means_to_global_means_comparison(xlabel='Principal Components', ylabel='Clusters',
                                                 ...
                                                 output_path=os.path.join("img_MAC", "聚类均值对比热图clustering_intra_comparison.jpg"))
print("已完成聚类均值对比热图的生成。") 

# 比较原始数值变量在各聚类中的分布
# Compare distributions of original variables by cluster
cl.plot_distribution_comparison_by_cluster(output_path=os.path.join("img_MAC", "比较原始数值变量在各聚类中的分布clustering_distribution_comparison.jpg"))
...
cl.plot_clusters_2D('dim_01', 'dim_02', output_path=os.path.join("img_MAC", "2D数值变量的可视化clustering_2d_plots.jpg"))
print("已完成2D数值变量的可视化的生成。")

# 比较单一原始分类变量在各聚类中的分布
# Comparison of original categorical variable distribution by cluster
print(cl.describe_clusters_cat(df_['ATBG12D'], cat_name='ATBG12D', normalize=True))

# 比较单一原始分类变量在各聚类中的分布:结果保存
cl.describe_clusters_cat(df_['ATBG12D'], cat_name='ATBG12D', normalize=True).to_csv('data_MAC/data_澳门_modified_比较单一原始分类变量在各聚类中的分布.csv', index=False)


# 单一原始分类变量在各聚类中的分布
cl.plot_cat_distribution_by_cluster(df_['ATBG12D'], cat_label='ATBG12D', cluster_label='Student clusters',
...
# 其中包含与原始数据相关的聚类。
df_.to_csv('data_MAC/data_澳门_modified_preprocessed_clustered_output.csv', index=False)
...
# 确保目标变量是整数
df_['cluster'] = df_['cluster'].astype(int)  # 新增代码
...
classifier.train_model(features_to_keep=["ASDGSEC", "ASDGSSB", "ASDGSB", "ASDGERL",
    "ASDGDRL", "ASDGSLR", "ASDGSCR", "ASDG05S", "ASDGHRL", "ASDHSES", "ASDHELA", "ASDHENA", "ASDHELN", "ASDHELT",
    "ASDHPCS", "ASDHPLR", "ASDHAPS", "ASDHEDUP", "ASDHOCCP", "ACDGRRS", "ACDGEAS", "ACDGDAS", "ACDGSBC", 
    "ATDGEAS", "ATDGSOS", "ATDGTJS", "ATDGSLI","ASBGSEC", "ASBGSSB", "ASBGSB", "ASBGERL", "ASBGDRL", "ASBGSLR", "ASBGSCR",
    "ASBGHRL", "ASBHSES", "ASBHELA", "ASBHENA", "ASBHELN", "ASBHELT", "ASBHPCS", "ASBHPLR","ACBGRRS", "ACBGEAS", "ACBGDAS", "ACDGTIHY",
...
classifier.plot_confusion_matrix(output_path=os.path.join("img_MAC", "混淆矩阵classifier_confusion_matrix.jpg"))
print("已完成混淆矩阵的生成。")
...
classifier.plot_roc_curves(output_path=os.path.join("img_MAC", "评估分类性能的ROC曲线classifier_roc_curves.jpg"))
...
classifier.plot_shap_importances_beeswarm(class_id=1,
                                          output_path=os.path.join("img_MAC", "聚类1的局部重要性classifier_local_importance_cl1.jpg"))
plt.clf()
print("已完成聚类1的局部重要性生成")

classifier.plot_shap_importances_beeswarm(class_id=2,
                                          output_path=os.path.join("img_MAC", "聚类2的局部重要性classifier_local_importance_cl2.jpg"))
plt.clf()
...
classifier.plot_shap_values_distribution(output_path=os.path.join("img_MAC", "SHAP值的分布图classifier_shap_values_distribution.jpg"))
plt.clf()
