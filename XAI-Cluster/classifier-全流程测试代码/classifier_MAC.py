...
# 全部数值主成分的主要贡献变量:结果保存
dr.num_main_contributors().to_csv('data_MAC/data_澳门_modified_全部数值主成分的主要贡献变量.csv', index=False)
...
# 解释从分类变量中提取的成分:结果保存
dr.cat_main_contributors().to_csv('data_MAC/data_澳门_modified_全部分类主成分的主要贡献变量.csv', index=False)
...
cl.describe_clusters_cat(df_['ASDHSES'], cat_name='ASDHSES', normalize=True).to_csv('data_MAC/data_澳门_modified_比较单一原始分类变量ASDHSES在各聚类中的分布.csv', index=False)
...
# 获取有多少类别，保存结果
df_['cluster'].value_counts().to_csv('data_MAC/data_澳门_modified_聚类类别数.csv', index=False)
...
# 查看模型超参数调优结果。保存结果
classifier.confusion_matrix().to_csv('data_MAC/data_澳门_modified_超参数调优结果.csv', index=False)
