...
# Load data # 加载数据
df = pd.read_csv('data_HKG/data_香港_modified.csv')
...
# 再次检查零方差数值变量
num_vars = [var for var in num_vars if df[var].var() > 1e-6]  # 更严格过滤

# 移除分类变量中唯一值总数小于2的分类变量 和 方差为零的分类变量
cat_vars_filtered = [var for var in cat_vars if df[var].nunique() >= 2 and df[var].var() > 0]
# 添加额外检查：删除唯一值数量为1的变量
cat_vars_filtered = [var for var in cat_vars_filtered if df[var].nunique() >= 2]
# 更新 num_vars 并筛选 DataFrame
...
# 缺失值比率结果保存
compute_missing(df).to_csv('data_HKG/data_香港_modified_缺失值比率.csv', index=False)
print("已完成，缺失值比率，结果保存。")

# 生成缺失值热图 Generate missing values heat map
missing_values_heatmap(df, output_path=os.path.join("img_HKG", "缺失值热图missing_heatmap.jpg"))
print("已完成缺失值热图的生成。")
...
# df_imp = impute_missing_values(df, num_vars=num_vars, cat_vars=cat_vars) # 限制相关性计算的变量数量，减少计算量：

# 修改 impute_missing_values 参数
...
plot_imputation_distribution_assessment(df.loc[df_imp.index], df_imp, list(compute_missing(df).head(16)['var_name']),
                                        output_path=os.path.join("img_HKG", "插补分布对比图imputation_distribution_assessment.jpg"))
print("已完成插补分布对比图的生成。")
...
# 插补结果保存
df_.to_csv('data_HKG/data_香港_modified_preprocessing.csv', index=False)
...
# 全部数值主成分的主要贡献变量:结果保存
dr.num_main_contributors().to_csv('data_HKG/data_香港_modified_全部数值主成分的主要贡献变量.csv', index=False)
print("已完成，全部数值主成分的主要贡献变量，结果保存。")

# 绘制全部数值变量的主要贡献变量图表
dr.plot_num_main_contributors(n_contributors=8,  # 显示前8个主要贡献变量
                              output_path=os.path.join("img_HKG", '全部数值主成分的主要贡献变量图表dim_red_num_contributors.jpg')
                              )
...
# 绘制主要贡献变量图表
dr.plot_num_main_contributors(dim_idx=3, output_path=os.path.join("img_HKG", "其中一个数值主成分的主要贡献变量图表dim_red_main_contributors.jpg"))
...
dr.plot_num_explained_variance(0.5, plots=['cumulative', 'ratio', 'normalized'],
                               output_path=os.path.join("img_HKG", "数值变量降维的解释方差曲线dim_red_explained_variance.jpg"))
...
dr.plot_cat_explained_variance(0.5, plots=['cumulative', 'ratio', 'normalized'],
                               output_path=os.path.join("img_HKG", "分类变量降维的解释方差曲线dim_red_explained_variance.jpg")) 
...
# 解释从分类变量中提取的成分:结果保存
dr.cat_main_contributors().to_csv('data_HKG/data_香港_modified_全部分类主成分的主要贡献变量.csv', index=False)


# 下表提供了原始值的每个值的成分均值和std。这有助于评估新组件是否正确地反映了原始分类变量的不同值。
print("解释从分类变量中提取的成分:\n", dr.cat_main_contributors_stats())
# 解释从分类变量中提取的成分:结果保存
dr.cat_main_contributors_stats().to_csv('data_HKG/data_香港_modified_解释从分类变量中提取的成分.csv', index=False)
...
dr.plot_cat_main_contributor_distribution(dim_idx=0, 
                                          ...
                                          output_path=os.path.join("img_HKG", "分类变量成分分布dim_red_cat_component.jpg"))
print("已完成分类变量成分分布的生成。")

df_t.to_csv('data_HKG/data_香港_modified_preprocessed_dim_red_output.csv', index=False)
...
cl.plot_score_comparison(output_path=os.path.join("img_HKG", "聚类算法性能的图表performance_comparison.jpg"))
...
cl.plot_clustercount(output_path=os.path.join("img_HKG", "聚类数量分布图cluster_count.jpg"))
print("已完成聚类数量分布图的生成。")
...
# 比较聚类内均值与全局均值:结果保存
cl.compare_cluster_means_to_global_means().to_csv('data_HKG/data_香港_modified_比较聚类内均值与全局均值.csv', index=False)
...
cl.plot_cluster_means_to_global_means_comparison(xlabel='Principal Components', ylabel='Clusters',
                                                 ...
                                                 output_path=os.path.join("img_HKG", "聚类均值对比热图clustering_intra_comparison.jpg"))
print("已完成聚类均值对比热图的生成。") 

# 比较原始数值变量在各聚类中的分布
# Compare distributions of original variables by cluster
cl.plot_distribution_comparison_by_cluster(output_path=os.path.join("img_HKG", "比较原始数值变量在各聚类中的分布clustering_distribution_comparison.jpg"))
...
cl.plot_distribution_comparison_by_cluster(df_ext=df_[['ASBGERL', 'ASBHSES']],
                                           output_path=os.path.join("img_HKG", "部分_比较原始数值变量在各聚类中的分布clustering_distribution_comparison.jpg"))
print("已完成部分比较原始数值变量在各聚类中的分布的生成。")


# 2-D plots # 二维图 2D 数值变量的可视化
cl.plot_clusters_2D('dim_01', 'dim_02', output_path=os.path.join("img_HKG", "2D数值变量的可视化clustering_2d_plots.jpg"))
...
cl.describe_clusters_cat(df_['ASDHSES'], cat_name='ASDHSES', normalize=True).to_csv('data_HKG/data_香港_modified_比较单一原始分类变量ASDHSES在各聚类中的分布.csv', index=False)
...
cl.plot_cat_distribution_by_cluster(df_['ASDHSES'], cat_label='ASDHSES', cluster_label='Student clusters',
                                    output_path=os.path.join("img_HKG", "单一原始分类变量在各聚类中的分布clustering_cat_comparison.jpg"))
...
cl.plot_optimal_components_normalized(output_path=os.path.join("img_HKG", "最优聚类数选择曲线clustering_elbow_curve.jpg"))
print("已完成最优聚类数选择曲线的生成。")

# 其中包含与原始数据相关的聚类。
df_.to_csv('data_HKG/data_香港_modified_preprocessed_clustered_output.csv', index=False)
...
# 获取有多少类别，保存结果
df_['cluster'].value_counts().to_csv('data_HKG/data_香港_modified_聚类类别数.csv', index=False)
...
classifier.plot_confusion_matrix(output_path=os.path.join("img_HKG", "混淆矩阵classifier_confusion_matrix.jpg"))
print("已完成混淆矩阵的生成。")
...
classifier.plot_roc_curves(output_path=os.path.join("img_HKG", "评估分类性能的ROC曲线classifier_roc_curves.jpg"))
print("已完成评估分类性能的ROC曲线的生成。")


# 使用 SHAP（SHapley Additive exPlanations）方法绘制全局特征重要性图
# Plot global feature importance
classifier.plot_shap_importances(output_path=os.path.join("img_HKG", "全局特征重要性图classifier_global_feature_importance.jpg"))
plt.clf()
print("已完成全局特征重要性图生成")
...
classifier.plot_shap_importances_beeswarm(class_id=1,
                                          output_path=os.path.join("img_HKG", "聚类1的局部重要性classifier_local_importance_cl1.jpg"))
plt.clf()
print("已完成聚类1的局部重要性生成")

classifier.plot_shap_importances_beeswarm(class_id=2,
                                          output_path=os.path.join("img_HKG", "聚类2的局部重要性classifier_local_importance_cl2.jpg"))
plt.clf()
...
# classifier.plot_shap_values_distribution(output_path=os.path.join("img_HKG", "SHAP值的分布图classifier_shap_values_distribution.jpg"))
# plt.clf()
