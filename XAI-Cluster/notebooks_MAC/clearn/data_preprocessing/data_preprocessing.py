...
def imputation_pairs(df, num_vars=None, cat_vars=None, num_kws=None, mixed_kws=None, cat_kws=None):
    ...
    # 修改为
    num_pairs_clean = num_pairs.dropna(axis=1, how='all')
    mixed_pairs_clean = mixed_pairs.dropna(axis=1, how='all')
    cat_pairs_clean = cat_pairs.dropna(axis=1, how='all')
    final_pairs = pd.concat([num_pairs_clean, mixed_pairs_clean, cat_pairs_clean], ignore_index=True)
...
def impute_missing_values(df, num_vars, cat_vars, 
                          num_pair_kws=None, mixed_pair_kws=None, cat_pair_kws=None,
