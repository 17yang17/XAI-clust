...
def plot_optimal_normalized_elbow(values, kl, ax, optimal_label='', xlabel='', ylabel=''):
    ...
    # print(f"kl.knee: {kl.knee}, n_components: {len(values)}")
    ...
    # 确保 kl.knee 为整数
    kl_knee = int(np.floor(kl.knee))
    # 绘制垂直线
    ax.axvline(kl_knee, linestyle='--', linewidth=1, color='#E73F74', label=optimal_label)

    # ax.set_xticks(np.append(ax.get_xticks()[1:-1], [kl.knee]))
    # ax.set_xlim(-n_components * 0.02, n_components * 1.02)
    # 设置横坐标刻度
    xticks = sorted(np.unique(np.append(ax.get_xticks(), kl_knee)))
    ax.set_xticks(xticks)
    # 确保横坐标范围覆盖 kl.knee
    ax.set_xlim(min(-n_components * 0.02, kl_knee - 1), max(n_components * 1.02, kl_knee + 1))
