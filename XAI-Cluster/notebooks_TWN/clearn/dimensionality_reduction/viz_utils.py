...
def plot_cat_main_contributor_distribution(df, df_trans, thres=0.14, n_contributors=5, dim_idx=None,
    ...
    # 获取主要贡献变量，并根据 n_contributors 截断
    ...
    #     ncols = 5
    if ncols > n_contributors:
        print(f"Info: {ncols} variables exceed threshold. Showing top {n_contributors}.")
        mc = mc.head(n_contributors)
        ncols = n_contributors
    # 计算行数
    # nrows = np.lcm.reduce(df[mc['var_name'].tolist()].nunique().tolist())
    # 计算行数：取分类变量的最大类别数
    nunique_list = df[mc['var_name'].tolist()].nunique().tolist()
    max_nunique = max(nunique_list) if nunique_list else 1
    nrows = max_nunique

    # fig = plt.figure(figsize=(np.minimum(6 * ncols, 30), 5))
     # 动态设置画布高度（每个类别分配0.8英寸）
    fig_height = max(5, 0.8 * max_nunique)  # 最小高度为5英寸
    fig = plt.figure(figsize=(np.minimum(6 * ncols, 30), fig_height))
    ...
    for idx, row in mc.iterrows():
        ...
        for v in np.sort(df[var_name].unique()):
            ...
            ax = fig.add_subplot(gs[i, j], sharex=ax0, sharey=ax0)  # 每类别占一行
            # sns.kdeplot(data=df[df[var_name] == v], x=dim_name, color='blue', fill=True, ax=ax)
            subset = df[df[var_name] == v]
            if subset[dim_name].var() == 0:
                print(f"Warning: Category '{v}' of variable '{var_name}' has zero variance. Skipping plot.")
                continue
            sns.kdeplot(data=subset, x=dim_name, color='blue', fill=True, ax=ax)
    ...
    sns.despine(fig)
    fig.supylabel('Density functions', fontsize=10, x=0.01)
