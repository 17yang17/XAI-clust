...
class DimensionalityReduction:
    ...
    def _transform_cat(self, df, n_components_cat=None):
        ...
        if n_components_cat is None and self.min_explained_variance_ratio_ is None:
            ...
            # 修改说明
            # 问题根源：
            # 原代码中 x 的范围基于 df.nunique().sum()（分类变量唯一值总和），而 y（explained_variance_ratio）的实际长度由主成分数量决定，导致二者长度不一致。
            # 解决方案：
            # 将 x 的范围改为 range(1, len(explained_variance_ratio) + 1)，确保 x 的长度与 y 完全一致。
            kl = KneeLocator(x=range(1, len(explained_variance_ratio) + 1),
                y=explained_variance_ratio,
                curve='convex',
                direction='decreasing')
    ...
    def plot_cat_main_contributor_distribution(self, thres=0.14, n_contributors=5, dim_idx=None, output_path=None,
                                               savefig_kws=None):
        """
        Plot main contributors (original variables with the strongest relation with derived variables) for
        every derived variable
